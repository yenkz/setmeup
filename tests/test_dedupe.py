...
def test_equal_score_within_batch_first_wins():
    candidates = [
        DedupeCandidate(track_id=1, dedupe_key="rec-1", quality_score=10),
        DedupeCandidate(track_id=2, dedupe_key="rec-1", quality_score=10),
    ]
    result = resolve_duplicates(candidates, existing={})
    assert result.winners == [1]
...
def test_existing_equal_score_makes_candidate_a_loser():
    candidates = [DedupeCandidate(track_id=1, dedupe_key="rec-1", quality_score=10)]
    result = resolve_duplicates(candidates, existing={"rec-1": 10})
    assert result.winners == []
    assert result.losers == [1]
